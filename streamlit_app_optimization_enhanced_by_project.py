...
import sys
from falcon_client import FalconClient, FalconSettings, ThanosSettings
...
def main():
    ...
    if project_id:
        try:
            ...
            logger.info(f"Project languages: {getattr(project_info, 'languages', [])}")
    ...
    if st.session_state.project_info:
        ...
        logger.info("Displaying project information from session state")
        logger.info(f"Session state project info: {project_info}")
        
        # More detailed attribute logging
        name = getattr(project_info, 'name', 'Unknown')
        description = getattr(project_info, 'description', 'No description available')
        languages = getattr(project_info, 'languages', [])
        
        logger.info(f"Displaying - Name: {name}")
        logger.info(f"Displaying - Description: {description}")
        logger.info(f"Displaying - Languages: {languages}")
